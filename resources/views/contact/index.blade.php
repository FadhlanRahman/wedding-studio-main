    ...
    <div class="max-w-6xl mx-auto px-6">
        ...
        <div class="grid lg:grid-cols-2 gap-12">
            ...
            <div class="bg-white p-8 rounded-2xl shadow-xl border border-blue-100 space-y-6 animate-fadeInUp" style="animation-delay:300ms;">
                ...
                {{-- Instagram --}}
                <p class="text-gray-700"><strong>Instagram:</strong>
                    @if(!empty($contact->instagram))
                        <a href="https://instagram.com/{{ ltrim($contact->instagram, '@') }}" target="_blank" class="text-pink-600 hover:underline">
                ...
                {{-- WhatsApp --}}
                <p class="text-gray-700"><strong>WhatsApp:</strong>
                    @if(!empty($contact->whatsapp))
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $contact->whatsapp) }}" target="_blank" class="text-green-600 hover:underline">
                            {{ $contact->whatsapp }}
                        </a>
                    @else
                        <span class="text-gray-500">Belum tersedia</span>
                ...
                {{-- Google Maps --}}
                <p class="text-gray-700"><strong>Google Maps:</strong>
                ...
                <div class="flex space-x-4">
                    {{-- Instagram --}}
                    @if(!empty($contact->instagram))
                        <a href="https://instagram.com/{{ ltrim($contact->instagram, '@') }}" target="_blank" class="text-pink-500 hover:text-pink-400 transition transform hover:scale-110">
                    ...
                    {{-- WhatsApp --}}
                    @if(!empty($contact->whatsapp))
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $contact->whatsapp) }}" target="_blank" class="text-green-500 hover:text-green-400 transition transform hover:scale-110">
                    ...
                    {{-- Google Maps --}}
                    @if(!empty($contact->map_url))
                        <a href="{{ $contact->map_url }}" target="_blank" class="text-red-500 hover:text-red-400 transition transform hover:scale-110">
