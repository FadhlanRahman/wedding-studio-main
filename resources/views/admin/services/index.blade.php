...
<div class="bg-white p-4 md:p-6 rounded-lg shadow-md">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-3">
        ...
        @if(session('success'))
            <div class="bg-green-100 text-green-700 p-3 rounded w-full md:w-auto text-center md:text-left">
                {{ session('success') }}
            </div>
        @endif
    </div>
        ...
        <table class="min-w-full border border-gray-200 rounded-lg text-sm md:text-base">
            <thead class="bg-gradient-to-r from-blue-600 to-blue-500 text-white">
                <tr>
                    ...
                    <th class="px-4 py-2 border text-left w-40">Actions</th>
                </tr>
            </thead>
                    ...
                    <td class="px-4 py-2 border">
                        <div class="flex flex-col sm:flex-row gap-2">
                            <a href="{{ route('admin.services.edit', $service) }}" 
                               class="bg-yellow-400 text-white px-3 py-1.5 rounded-lg hover:bg-yellow-500 text-xs md:text-sm text-center">
                                ✏️ Edit
                            </a>
                            <form action="{{ route('admin.services.destroy', $service) }}" 
                                ...
                                @csrf @method('DELETE')
                                <button class="bg-red-500 text-white px-3 py-1.5 rounded-lg hover:bg-red-600 text-xs md:text-sm w-full sm:w-auto">
                                    🗑️ Delete
                                </button>
                            </form>
                        </div>
